...
def show_rates(currency):
    ...
    if not os.path.exists(f"x_rates.json"):
        parser()
    if os.path.exists(f"x_rates.json"):
        ccy_indices = {"USD": [0, 1], "EUR": [2, 3], "RUB": [4, 5], "KZT": [6, 7]}
        with open(f"x_rates.json", "r", encoding="utf-8") as json_file:
...
def parser():
    ...
    for row in rows:
        ...
        with open(f'x_rates.json', 'w', encoding="utf-8") as xf:
...
def my_scheduler():
    ...
    schedule.every().hour.at(":58").do(parser)
    while True:
